#! /usr/bin/env python
# python  ./mlfq.py  -q 5 -c -f True -m 10
...
parser.add_option('-f', '--arrjobFront', default=False,
                  help='',
                  action='store_true', dest='arrjobFront')
    ...
    if currTime in ioDone:
        for (j, type) in ioDone[currTime]:
            ...
            if options.iobump == False or type == 'JOB BEGINS':
                # job이 처음 arrive한 경우 arrjobFront option이 false면 기존의 코드와 같이 append를 통해 queue의 맨 뒤에 job을 추가하고
                # arrjobFront option이 true인 경우에는 insert(0,j)를 통해서 queue의 맨 앞에 해당 job이 배치되도록 하였다.
                if options.arrjobFront == False: queue[q].append(j)
                else: queue[q].insert(0,j)
            else:
                queue[q].insert(0, j)
